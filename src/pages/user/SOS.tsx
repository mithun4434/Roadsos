...
import { AlertTriangle, Loader2, Truck, Fuel, CircleDot, Zap, Wrench } from 'lucide-react';
...
const driverActiveIcon = L.divIcon({
  className: 'bg-transparent border-none animated-marker',
...
function DynamicBounds({ userLoc, driverLoc }: { userLoc: {lat: number, lng: number}, driverLoc: {lat: number, lng: number} | null }) {
  const map = useMap();
  useEffect(() => {
    if (driverLoc) {
      ...
      map.fitBounds(bounds, { animate: true, padding: [60, 60] });
    } else if (userLoc.lat !== 0) {
...
export default function SOS() {
  ...
  const serviceName = searchParams.get('name') || 'Emergency Help';
  const serviceIconName = searchParams.get('icon') || 'alert';
  const ServiceIcon = ICONS[serviceIconName] || AlertTriangle;
    ...
    <div className="flex flex-col items-center justify-center min-h-[80vh] p-4 text-center">
      <AnimatePresence mode="wait">
        {status === 'IDLE' && (
          ...
          >
            <div className="mb-8">
              <h1 className="text-3xl font-bold text-red-500 mb-2">Requesting {serviceName === 'Emergency Help' ? 'Help' : serviceName}</h1>
              <p className="text-white/60">Tap the button below to instantly broadcast a request for {serviceName}.</p>
            </div>
            ...
            >
              <ServiceIcon className="w-20 h-20 text-white group-hover:scale-110 transition-transform drop-shadow-[0_0_10px_rgba(255,255,255,0.8)]" />
            </button>
          </motion.div>
        )}
        ...
        {status === 'SEARCHING' && (
          ...
          >
             ...
             <p className="text-white/60 mb-6">Locating the nearest {serviceName} specialist.</p>
          ...
          >
             <div className="flex-1 rounded-[32px] overflow-hidden glass p-1 shadow-2xl relative border border-white/20 z-0">
               <MapContainer
                  ...
                  zoomControl={false}
               >
                 <DynamicBounds userLoc={userLoc} driverLoc={driverLoc} />
               ...
               <div className="absolute bottom-4 left-4 right-4 z-[400] glass-dark rounded-[24px] p-4 shadow-2xl border border-white/10">
                 ...
                 <p className="text-white/60 text-sm mb-4">A {serviceName} specialist is actively navigating to your coordinates.</p>
                 ...
                 <button 
                   ...
                   className="w-full bg-red-600/20 text-red-500 border border-red-500/30 py-3 rounded-xl hover:bg-red-600/40 transition-colors font-bold uppercase tracking-wider text-xs"
                 >
                   Cancel Request
                 </button>
               </div>
