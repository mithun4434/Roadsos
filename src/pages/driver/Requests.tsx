...
function DynamicBounds({ destLoc, myLoc }: { destLoc: {lat: number, lng: number}, myLoc: {lat: number, lng: number} }) {
  const map = useMap();
  useEffect(() => {
    if (destLoc && myLoc.lat !== 0) {
        ...
        <div className="p-4 space-y-4 pb-32 animate-in slide-in-from-bottom-10 fade-in duration-500">
          ...
          <section className="relative w-full rounded-[24px] overflow-hidden glass shadow-2xl p-1 border border-orange-500/50 z-0 h-[300px]">
            <MapContainer
              ...
              zoomControl={false}
            >
              <DynamicBounds destLoc={activeJob.user_location} myLoc={myLoc} />
