...
export const AuthProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  useEffect(() => {
    const initializeAuth = async () => {
      try {
        ...
        if (error) {
          console.warn("Session error detected, clearing corrupted auth states:", error);
          if (error.message.includes('Refresh Token') || error.message.includes('Fetch')) {
            await supabase.auth.signOut().catch(() => {});
            // Force clear corrupted local storage if Supabase signOut gets stuck
  ...
  const handleSession = async (session: Session | null) => {
    try {
      ...
      if (error) {
         if (error.message.includes('Fetch') || error.message.includes('fetch')) {
             console.warn("Network error or invalid Supabase connection during role fetch:", error);
             // Stay logged in conceptually but perhaps flag a network error
             // Alternately, clear the session if the auth backend is totally mismatched
         }
      }
  ...
  const signInWithEmail = async (email: string, password: string, isSignUp: boolean) => {
    if (!import.meta.env.VITE_SUPABASE_URL) {
      return { error: new Error("Please configure VITE_SUPABASE_URL in AI Studio settings.") };
    }
